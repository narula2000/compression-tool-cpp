...
- [ ] Add missing `i++` in `-m`/`--mode` CLI parser (main.cpp:125-131)
- [ ] Initialize `character` to `'\0'` in `Node(int frequency)` constructor (main.cpp:50)
- [ ] Remove shadowed `char c;` in `build_counter_map` (main.cpp:32)
- [ ] Fix memory leaks: free Huffman tree nodes after use
...
- [ ] Replace `read_file_content` with `std::istreambuf_iterator`

## Style & Robustness
- [ ] Use `std::cerr` for error messages
- [ ] Pass strings by `const&` in `build_counter_map`
- [ ] Use `const auto&` in range-for loops
